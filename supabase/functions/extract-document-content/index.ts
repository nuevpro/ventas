...
serve(async (req) => {
  ...
  try {
    const { fileContent, fileName, fileType, fileId } = await req.json()
    
    if (!fileContent && !fileId) {
      throw new Error('File content or file ID is required')
    }
    ...
    // Si tenemos fileId pero no fileContent, intentamos recuperar el archivo de la base de datos
    if (fileId && !fileContent) {
      console.log('Attempting to reprocess file with ID:', fileId)
      
      // Aquí podríamos intentar obtener el contenido del archivo desde storage
      // Por ahora, continuamos con el procesamiento sin el contenido original
      extractedText = `Reintento de procesamiento para el documento ${fileName}. `
    }
